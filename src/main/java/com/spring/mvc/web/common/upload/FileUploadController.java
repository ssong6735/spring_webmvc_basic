...
public class FileUploadController {
    ...
    // 업로드 파일 저장 경로
    private static final String UPLOAD_PATH = "D:\\developing_ssong\\upload";

    // upload-form.jsp 를 열어주는 처리
    ...
    // URI: /deleteFile?fileName=/2021/06/09/adfadf_abc.txt
    ...
    public ResponseEntity<String> deleteFile(String fileName) throws IOException {
        ...
        try {
            ...
            if (isImage(fileName)) {
                // 원본을 지우는 코드
                // fileName => 썸네일 이미지 경로: /2021/06/09/s_asdfasdfdfads_abc.jpg
                // originalName => 원본 이미지 경로: /2021/06/09/asdfasdfdfads_abc.jpg
                int lastSlachIdx = fileName.lastIndexOf("/");
                String originalName = fileName.substring(0, lastSlachIdx + 1)
                        + fileName.substring(lastSlachIdx + 3);
    ...
    private boolean isImage(String fileName) {
        /*String ext = FileUtils.getFileExtension(fileName);
        MediaType mediaType = FileUtils.getMediaType(ext);
        return FileUtils.getMediaType() != null;*/
