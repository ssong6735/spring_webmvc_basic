...
public class FileUtils {
    ...
    // 1. 사용자가 파일을 업로드 했을 때 저장처리 및 파일명을 리턴하는 메서드
    public static String uploadFile(MultipartFile file, String uploadPath) throws IOException {

        // 중복없는 파일명으로 변환
        // ex) dog.jpg -> 325hljk5hlkj2kl3h5lk3klj5lkh23kl4jlkh35klh5_dog.jpg
        ...
        /*
            만약 업로드한 파일이 이미지파일 이라면? 추가로 썸네일을 생성해서 저장하고
            해당 썸네일의 경로를 클라이언트에 응답 (저장은 원본, 보여주는건 썸네일)
            이미지가 아니라면? 그냥 다운로드 할 수 있게 일반 파일경로를 리턴
        */

        // 클라이언트가 서버에있는 파일을 로딩하려면 해당 파일이 어디에 저장되어있는지 정보가 필요하다.
        // 따라서 중복되는 경로를 제외한(d:/~/upload) 경로를 리턴해야 함.
        // 응답할 파일명: /2021/~~/파일명
        // .substring(uploadPath의 길이)부터 끝까지 잘라내기
        // ex) D:/developing/upload/2021/06/07
    ...
    public static String getFileExtension(String fileName) {
        // ex) fileName: 325hljk5hlkj2kl3h5lk3klj5lkh23kl4jlkh35klh5_dog.jpg
